...
//MoveWindow(Conosle, r.left, r.top, width, height,TRUE);
}
...
void menu()
{
	//system("cls");
	borderNoDelay();
	//cout<<"Number of Records Available : "<<num;
	gotoXY(16,4);
	printf("******* Payroll Management System v1.0 ******");
	gotoXY(12,6);
...
void search()
{
	...
	for(int i=0;i<=num-1;i++)
	{
		...
		{
			gotoXY(6,8);
			cout<<"Name\tCode\tDesignation\tYear(EXP)\tAge\tSalary "<<endl;
			gotoXY(6,9);
			cout<<"----------------------------------------------------------------"<<endl;
			gotoXY(6,11);
			cout<<emp[i].name;
...
{
	gotoXY(10,18);
	cout<<"Enter new Years of Exprience ------> ";
	cin>>emp[i].exp;
}
